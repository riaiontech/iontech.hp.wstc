(function(){
  ...
  function renderFamilySelection(){
    ...
    if(!box)return;
    box.innerHTML=FAMILY_GROUPS.map(group=>`
      <div class="selection-group">
        <div class="selection-title">${esc(group.category)}</div>
        <div class="selection-buttons">
          ${group.items.map(([label])=>`<button class="selection-btn ${selectedFamily===label?"active":""}" data-family="${esc(label)}">${esc(label)}</button>`).join("")}
        </div>
      </div>`).join("");
  }
  ...
  function render(){
    ...
    let list=products.filter(p=>{
      ...
      return (!q||hay.includes(q)) && (!selectedFamily||familyFor(p)===selectedFamily);
    });
